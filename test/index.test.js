...
describe('starkdown()', () => {
	describe('text formatting', () => {
		it('parses bold with **', () => {
			expect(starkdown('I **like** tiny libraries')).toEqual('I <strong>like</strong> tiny libraries');
		});

		it('parses bold with __', () => {
			expect(starkdown('I __like__ tiny libraries')).toEqual('I <strong>like</strong> tiny libraries');
		});
	...
	describe('links & images', () => {
		it('parses links', () => {
			expect(starkdown('[starkdown](http://github.com/developit/starkdown)')).toEqual('<a href="http://github.com/developit/starkdown">starkdown</a>');
		});
		...
		it('parses images within links', () => {
			expect(starkdown('[![](toc.png)](#toc)')).toEqual('<a href="#toc"><img src="toc.png" alt=""></a>');
			expect(starkdown('[![a](a.png)](#a) [![b](b.png)](#b)')).toEqual('<a href="#a"><img src="a.png" alt="a"></a> <a href="#b"><img src="b.png" alt="b"></a>');
		});
	...
	describe('code & quotes', () => {
		it('parses inline code', () => {
			expect(starkdown('Here is some code `var a = 1`.')).toEqual('Here is some code <code>var a = 1</code>.');
		});
		...
		it('parses three backtricks (```) as a code block', () => {
			expect(starkdown('```\nfunction codeBlocks() {\n\treturn "Can be inserted";\n}\n```')).toEqual('<pre class="code "><code>function codeBlocks() {\n\treturn &quot;Can be inserted&quot;;\n}</code></pre>');

			expect(starkdown('```js\nfunction codeBlocks() {\n\treturn "Can be inserted";\n}\n```')).toEqual('<pre class="code js"><code class="language-js">function codeBlocks() {\n\treturn &quot;Can be inserted&quot;;\n}</code></pre>');
		});

		it('parses tabs as a code poetry block', () => {
			expect(starkdown('\tvar a = 1')).toEqual('<pre class="code poetry"><code>var a = 1</code></pre>');
		});

		it('escapes code/quote blocks', () => {
			expect(starkdown('```\n<foo>\n```')).toEqual('<pre class="code "><code>&lt;foo&gt;</code></pre>');
	...
	describe('edge cases', () => {
		it('should close unclosed tags', () => {
			...
			expect(starkdown('[some **bold text](#winning)')).toEqual('<a href="#winning">some <strong>bold text</strong></a>');
